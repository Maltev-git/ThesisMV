...
The differential rotation of the solar surface is visible from sunspot observations and was first discovered by \citet{Scheiner1630}. It is caused by transport of angular momentum away from the rotation axis resulting from the Sun's inner thermal convective circulation.

%rotation period
\citet{Bartels1934} set the average synodic solar rotation period to 27~days for the definition of his solar rotation number. The Bartels Rotation Number counts the solar rotations starting with 8~February 1832. Carrington determined a more accurate average solar rotation period of 27.2753~days, which is valid for solar latitudes of \SI{+-16}{\degree} where sunspots usually are found. He defined the Carrington Rotation Number based upon this period, starting with 9~November 1853 (cite?).\\
% sidereal: 25.38~d (of 609.12~h Sun Fact Sheet...), synodic: 27.2753~d (derived)

%best-fitting function
The Sun's sidereal differential angular velocity is described by the best-fitting function\footnote{NASA's Sun Fact Sheet: \urlfoot{http://nssdc.gsfc.nasa.gov/planetary/factsheet/sunfact.html}}:
\begin{align}
	\omega_\odot(\theta) = \omega_\text{eq} + B \sin^2(\theta) + C \sin^4(\theta)	\,,	\label{eq:omega_differential}
\end{align}
with the heliolatitude $\theta$, the equatorial angular velocity $\omega_\text{eq} = \SI{14.37}{\degree\per\day}$, the coefficients $B = \SI{-2.33}{\degree\per\day}$, and $C = \SI{-1.56}{\degree\per\day}$. The function is plotte in \autoref{fig:differential_rotation_pdfcairo_plot}.
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		\includegraphics[width=0.6\textwidth]{figures_of_mine/gnuplots/differential_rotation_pdfcairo_plot.pdf}
	}{
		\caption{Diagram showing the angular velocity of the sidereal solar surface differential rotation for different latitudes. remove sides and y2axis... +x2axis with rotation period?}
...
\begin{align}
	...
	&= \SI{26.90}{\day}	\,,	\nonumber
\end{align}
with the Earth's orbital rotation period $T_\text{E} = \SI{365.25}{\day}$ (1/100 Julian century).\\
...
\begin{table}[htb]\small
	...
	\begin{tabular}{llll}
		\hline\hline
		Rotation	&Equator	&\SI{+-16}{\degree}~latitudes	&Poles\\
...
Pulkkinen1998:\\
the solar differential rotation is continuously changing. During the present century, the rotation rate seems to have been slowing down, and so the concept of a global mean differential rotation may not be fully applicable.
...
All interactions between Sun and Earth are governed by the geometry of Earth's orbit and the orientations of both bodies' rotation axes. The Earth orbit defines the ecliptic plane which sets a convenient basis for a few coordinate systems.
...
The orientation of the Earth's rotation axis to the Sun defines the solar wind flow angle onto the magnetosphere. The Earth's obliquity of the ecliptic is \SI{23.439279}{\degree} \citep{Luzum2011}.\\

The seasonal change, seen in \autoref{fig:Sun_Earth_geometry}, is reflected in geomagnetic indices as well.
...
\end{align*}
Further useful characteristics are the position of the median and the mode:
\begin{align*}
...
Solar rotation axis tilt to ecliptic: $i_\odot = \SI{7.25}{\degree}$ (Sun fact sheet...)\\ %C3; Obliquity to ecliptic
...
Online resources:\\
IAU Division I Working Group, Numerical Standards for Fundamental Astronomy, IAU 2009 values and best estimates: \urltext{http://maia.usno.navy.mil/NSFA/}\\
\citep{Luzum2011}\\

The Astronomical Almanac Online: \urltext{http://asa.usno.navy.mil/static/files/2016/Astronomical_Constants_2016.pdf}\\
\citep{USNAO2017}\\

NASA's Planetary~Fact~Sheets: \urltext{https://nssdc.gsfc.nasa.gov/planetary/planetfact.html}}\\
