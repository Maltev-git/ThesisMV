...
The Sun's sidereal differential angular velocity can be approximated by the function \citep{Beck2000}:
\begin{align}
...
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		...
		\caption[\lofimage{figures_of_mine/gnuplots/differential_rotation.pdf}I created the figure myself.]
		{Diagram showing the angular velocity of the sidereal solar surface differential rotation over solar latitude. The upper x-axis shows the corresponding sidereal rotation period.}
...
\end{align}
The plasma at the photosphere has typical beta values around~14 and that of the low corona around~0.2 \citep{Gary2001}. Further up, beta rises again and the region where it equals~1 is defined as the source surface for the solar wind \citep{Schatten1969}.	% source surface where beta equals 1 (Schatten1969), at 0.6~Rs\\
...
The Sun--Earth distance can be derived from the orbital parameters of Earth\footnote{NASA/JPL Solar System Dynamics, Keplerian Elements: \urlfoot{https://ssd.jpl.nasa.gov/?planet_pos}}, the semimajor axis $a = \SI{1}{\au}$ and the eccentricity $e = \num{0.0167}$. Hence, the perihelion and aphelion distances are:
\begin{align*}
...
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		...
		\caption[\lofimage{figures_of_mine/gnuplots/Sun_Earth_geometry.pdf}I created the figure myself.]
		{Seasonal changes of the Sun's and the Earth's tilt angles to the ecliptic and of the Earth's solar distance. The curves are approximated with trigonometric functions.}
...
Over one year, the position of Earth oscillates within an HGI latitude range of \SI{+-7.25}{\degree}.

HGI coordinates are useful when looking at processes concerning the solar wind in the heliosphere. These coordinates are applied for the processing of the Helios data, e.g., in Figures~\ref{fig:Helios_r_b_ssn} and \ref{fig:Helios12_orbits_ecliptic_polar}.
...
\begin{figure}[htb]
	\begin{floatrow}
		\ffigbox{
			...
			\caption[\lofimage{figures_of_mine/gnuplots/lognormal_semi_log.pdf}I created the figure myself.]
			{Lognormal probability density function plotted in a linear (top panel) and semi-log (bottom panel) way. Both distributions have the parameters $\sigma = 1$ and $\mu = 0$.}
			\label{fig:lognormal_semi_log}
		}
		\ffigbox{
			\includegraphics[width=0.46\textwidth]{figures_of_mine/gnuplots/lognormal_ms_b.pdf}
		}{
			\caption[\lofimage{figures_of_mine/gnuplots/lognormal_ms_b.pdf}I created the figure myself.]
			{Sets of five different lognormal distributions plotted with a fixed $\sigma$ (top panel) and a fixed $\mu$ (bottom panel). The Gaussian curve is drawn for comparison.}
