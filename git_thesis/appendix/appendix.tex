...
Some important ratios can be derived from this table: The probabilities of detection and of false detection (i.e., the hit rate and the false alarm rate)
\begin{align}
...
\begin{align}
	\mathit{FAR} = \frac{n_\text{FA}}{n_\text{FA} + n_\text{H}}	&	&\text{and}	&	&\mathit{DFR} = \frac{n_\text{M}}{n_\text{M} + n_\text{N}}	\,.
\end{align}

The TSS is the difference between the hit rate and the false alarm rate\footnote{False alarm rate -- not to be confused with false alarm ratio.} and thus the TSS is calculated as follows \citep[Eq.~15]{Hanssen1965}:
\begin{align}
