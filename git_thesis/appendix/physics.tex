...
There exists an incompressible wave mode within MHD plasmas which is a result of bending magnetic field lines \citep{Alfven1942}, now called shear Alfvén wave. Alfvén waves transport energy from the photosphere outwards and it is believed that they play a major part in the coronal heating and solar-wind accelerating process. Alfvén waves are dominant in coronal regions that have open magnetic field lines and thus are prevalent in fast solar wind \citep{Cranmer2005}. Their propagation velocity is an important parameter to characterize a plasma. In an ideal incompressible MHD plasma (viscosity $\mu = 0$ and electrical conductivity $\sigma = \infty$) the kinetic and magnetic energy density are of equal value \citep[p.~51]{Kivelson1995}: 
\begin{align}
	w_\text{kin} &= w_\text{mag}\\
	\frac{\rho v^2}{2} &= \frac{B^2}{2 \mu_0}	\nonumber
\end{align}
with the permeability constant $\mu_0$ and the total mass density $\rho$ of the charged plasma particles. Thus, Alfvén waves propagate with the Alfvén velocity
\begin{align}
...
\end{align}
with $\theta$ as the angle between wave propagation direction and magnetic field line, that is, Alfvén waves travel along magnetic field lines. They consist of periodic disturbances in the magnetic field orthogonal to its direction, in the electric field, the plasma velocity, and the current density. Plasma density, pressure and magnetic field magnitude are not affected by them \citep[p.~51]{Kivelson1995}.

Within solar wind at \SI{1}{\au}, their typical frequency is 1--4 per hour and their average velocity is $v_\text{A} = \SI{56.8}{\km\per\s}$ \citep{Veselovsky2010}.	% at 1~au in 1963--2007\\

Additionally, there exist two types of compressional wave modes within MHD plasmas, the fast-mode wave and the slow-mode wave. The phase speeds of the three MHD waves meet $v_\text{fast} \geq v_\text{A} \geq v_\text{slow}$. %block copy from Kivelson1995

Alfvén critical surface...\\
sonic critical surface...\\

%Cranmer2005 - ON THE GENERATION, PROPAGATION, AND REFLECTION OF ALFVÉN WAVES FROM THE SOLAR PHOTOSPHERE TO THE DISTANT HELIOSPHERE
