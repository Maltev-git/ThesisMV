...
If the solar rotation axis tilt to the ecliptic normal is neglected and the radial field component $B_0$ is set to be in the ecliptic ($\theta = 90\degree$) at $r_0 = \SI{1}{\au}$, then
\begin{align}
	B(r) &= B_0 \cdot \sqrt{r^{-4} + \left(\frac{\omega\,r}{v_\text{sw}}\right)^2 r^{-4}}\,,	\label{eq:B_1au}
\end{align}
with the distance $r$ in units of [au].\\

In the ecliptic at a distance of $r = \SI{1}{\au}$, the Parker spiral's observed magnetic field angle $\phi_B$ is centered most of the time at \SI{-45}{\degree} or in the opposite direction at \SI{135}{\degree}. This is apparent from its frequency distribution using OMNI data of the time period 1963--2016, see \autoref{fig:histogram_B_angles}.\\
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		\includegraphics[width=0.5\textwidth]{images/gnuplots/histogram_B_angles.pdf}
	}{
		\caption{Magnetic field angles $\theta_B$ and $\phi_B$ in GSE coordinates.}
		\label{fig:histogram_B_angles}
	}
\end{figure}
The angle can be calculated via...
\begin{align}
	\phi_B(\SI{1}{au}) &= \arctan\left(-\frac{\omega \cdot \SI{1}{au}}{v_\text{sw}} + 180\right)\,.
\end{align}
With the mean values $\omega = \SI{14.37}{\degree\per\day}$ (see appendix~XX) and $v_\text{sw} = 6.16$ (see paper) its value is: XX.\\
