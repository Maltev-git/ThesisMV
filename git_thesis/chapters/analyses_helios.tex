...
In the following a distance dependency for the IMF model of the paper is derived that is possibly more accurate in the near-Sun region. In any case, it is found that Voyager~1 observations of the magnetic field strength at solar distances larger than \SI{1}{\au} agree well with Parker’s IMF model \citep{Burlaga2002}.
...
For $\phi_B$ being in average centered around these two directions, both vector components $\vect{B}_r$ and $\vect{B}_\phi$ have to be of equal amplitude. It can be seen from Equations~\ref{eq:B_absolute} and \ref{eq:B_1au} that this leads to
\begin{align}
	B(\SI{1}{\au}) &= B_0 \cdot \sqrt{2}	&\text{with}	&	&	\left(\frac{\omega\cdot\SI{1}{\au}}{v_\text{sw}}\right) &= 1\,.
\end{align}
Admittedly, this way I treat $\omega$ and $v_\text{sw}$ as time independent constants.
...
\end{figure}
Already little above \SI{1}{\au}, the power-law yields significantly lower IMF strenghts. It is known that the decline in IMF strength in the range \SIrange{1}{81}{\au} behaves according to Parker's theory \citep{Burlaga2002}. However, both curves do not deviate that much in the PSP distance range.
...
The estimated frequency distributions of the solar wind magnetic field strength at PSP's 1st and 22nd (the first closest) perihelion are plotted in \autoref{fig:PSP_sw_distributions_c}.
\begin{figure}[htb]
	\fcapside[\FBwidth]{
