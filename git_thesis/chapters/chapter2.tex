...
The model which is based on the \vBz{} relation is more accurate than those based purely on the velocity. This is expected, however, the \vBz{} relation is still eclipsed by the persistence model.\\

% true skill score
In order to further test the derived models for their predictive value, I derive their true skill statistic (TSS) -- a common tool for forecast verification. The TSS is a skill score based on the contingency table that categorizes forecasted and observed events. It is the difference between the forecast hit rate and the false alarm rate. I give a more detailed description on the TSS in the appendix \autoref{sec:true_skill_statistic}.\\

This method is the prevalent form of forecast verification in \Kp{} models \citep{Detman1999,Wing2005,Savani2017}.\\

Here, each single 3"~hour \Kp{} interval represents an event and a hit occurs when both the forecasted and observed $\Kp{} \geq 5$.\\
I chose this threshold because of the NOAA/SWPC geomagnetic storm definition: Their G"~scale starts with G1, which corresponds to a \Kp{} of 5.\\

The derived true skill scores are plotted in \autoref{fig:true_skill_score}.\\
\begin{figure}
	\fcapside[\FBwidth]{
...
A statistics summary of the persistence and the three derived models is presented in \autoref{tab:model_statistics_table}.\\
\begin{table}[htb]
	...
	\begin{tabular}{lcccc}
		\hline\hline
		\multirow{2}{*}{Statistik}	&\Kp{} persistence	&E-field	&CME velocity 	&Stream velocity\\
			&$\Kp(t-1)$	&$\Kp(\text{\vBz})$	&$\Kp(v_\text{CME})$	&$\Kp(v_\text{Streams})$\\
		\hline
		Time shift [hours]	&3	&0	&0	&9\\
		Correlation coefficient	&0.81	&-0.72	&0.51	&0.66\\
		Valid \Kp{} range	&0.3--8.7	&1.0--8.7	&1.3--6.3	&0.3--4.3\\
		\hline
		Event count	&105192	&79276	&12116	&65774\\
...
\citet{Detman1999}:\\
``However, a persistence forecast has no real warning value since it only predicts a large Kp after a large Kp was observed. The NN pKp prediction driven by solar wind data from the L1 location offers about a quarter to half hour of warning time before a large increase in geomagnetic activity.''\\

``Kp is slightly more predictable near solar maximum than it is near solar minimum.'' \citet{Wing2005}, into outlook...\\

``We also note that r, by itself, does not indicate how well the model performs. For example, it would be possible to have a model that systematically and consistently underpredicts Kp but does so with little scatter and high r. Thus comprehensive model evaluations should include scatterplots (e.g., Figures 2, 3, 6, 7, 8), skill scores, and r.'' \citet{Wing2005}\\
...
Wing APL model 1 (1 h, input: Kp nowcast + sw): about 0.8\\
Wing APL model 2 (4 h, input: Kp nowcast + sw): about 0.7\\
Wing APL model 3 (1 h, input: sw): about 0.7\\
