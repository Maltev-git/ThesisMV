...
\begin{figure}[htb]
	\begin{floatrow}
		\ffigbox{
			...
			\caption{Solar wind velocity frequency distributions for the 3"~hour average and 3"~hour maximum data. The minutely OMNI data from the period 1981--2016 is used.}
...
\begin{figure}
	\fcapside[\FBwidth]{
		...
		\caption{CME part of the \Kp--velocity distribution (same as third panel of \autoref{fig:Kp_2dhistogram_V_sws_d}) and its relative distribution per velocity interval with the mean \Kp{} values (solid line) and their MAD (dotted lines). The bin size is \SI{10}{\km\per\s} and \SI{1/3}{\Kp}~unit respectively.}
...
%determine fitting function
Again, I use the logarithmic function (\ref{eq:log_offset_fit_function}) for the fitting process. The resulting fit parameters are $a = \num{5.88(38)}$, $x' = \num{2.99(49)e2}$, and $y' = \num{-3.70(29)e1}$, with the velocity in units of [\si{\km\per\s}].
%a1 = 5.88(38)
%b1 = -37.0(29)
%x1 = 299(49)
...
\begin{figure}
	\fcapside[\FBwidth]{
		...
		\caption[\lofimage{figures_of_mine/chapter2/model_performance_d.pdf}]
...
\begin{figure}
	\fcapside[\FBwidth]{
		...
		\caption{True skill scores for the \Kp{} persistence and the three predictive models as a function of \Kp{} threshold. The scores are calculated from the same data and time range as the relations themselves.}
...
The 2"~month sample period of solar wind and \Kp{} data is displayed in \autoref{fig:example_sw_plot_1month_b_2013-5-1_65}.
\begin{figure}[htb]
	\centering
	\includegraphics[width=\textwidth]{figures_of_mine/chapter2/example_sw_plot_1month_b_2013-5-1_65.pdf}
	\caption[\lofimage{figures_of_mine/chapter2/example_sw_plot_1month_b_2013-5-1_65.pdf}]
	{Solar wind parameters, official \Kp~index, and estimated \Kp{} indices for the 65"~day time period from 1~May to 5~July 2013. The solar wind parameters are the magnetic field strength, its z"~component in GSM coordinates, the velocity, and the product of the latter two. I also plot the velocity's 3"~hour maxima and the \vBz{}'s 3"~hour minima for illustration. The \Kp{} estimates based on the electric field and the velocity relations are displayed together with their mean MAD bands. The \Kp{} estimates from both velocity relations are chosen according to the periods flagged as CMEs or streams in the SWS list (red lines in top panel), that is, either via Equation~(\ref{eq:kpvsv_CME_dependency_function}) or (\ref{eq:kpvsv_stream_dependency_function}). The solar wind data are from the minutely OMNI data set and the official \Kp~index is obtained from the GFZ~Potsdam.}
	\label{fig:example_sw_plot_1month_b_2013-5-1_65}
...
% 6~day sub-period
I calculate the \Kp{} estimate for the CME from 27~June 2013 (see also \autoref{fig:ACE_64s_v7_thesis_CME_2013-6-26_6}) and include the 3"~hour minima values of \vBz{} in the plot, see \autoref{fig:example_sw_plot_CME_b_2013-6-26_6}.
\begin{figure}[htb]
	\centering
	\includegraphics[width=\textwidth]{figures_of_mine/chapter2/example_sw_plot_CME_b_2013-6-26_6.pdf}
	\caption[\lofimage{figures_of_mine/chapter2/example_sw_plot_CME_b_2013-6-26_6.pdf}]
	{Solar wind parameters, official \Kp~index, and estimated \Kp{} indices for the 6"~day time period from 26~June to 2~July 2013. All panels show the same parameters as in \autoref{fig:example_sw_plot_1month_b_2013-5-1_65}. The period covers the CME from 27~June 2013, which induced a peak \Kp{} of $6+$.}
	\label{fig:example_sw_plot_CME_b_2013-6-26_6}
	\addtocontents{lof}{\smallskip\protect\center I created the figure myself.\medskip}
\end{figure}
It can be seen that in the considered period the \Kp{} estimate from \vBz{} traces the actual \Kp~index pretty well within the mean MAD band. However, deviations are found at the times of the initial shock, and the start and middle of the MC.\\

The \Kp{} estimate derived from both velocity relations is overestimating all small \Kp{} values and underestimates the main part of the MC. However, it is able to track the initial shock when considering the initial mixed-flag 3"~hour interval. The velocity models do not coincide well during the whole period. The \Kp{} estimate performs okay during the initial sheath region and around the peak of the trailing HSS. The rarefaction regions of declining velocity at the start and end of the considered interval are overestimated, whereas the MC and the trailing compression region are underestimated.\\

% learned/conclusions
It becomes apparent how important the influence of the magnetic field z"~component is, when comparing the \vBz{} and velocity estimations.\\


% extreme CME speeds
CMEs can be faster than the maximal velocities included in the OMNI data. Current solar wind plasma spectrometers are only able to reliably measure speeds up to about \SI{1100}{\km\per\s}, leading to data gaps during these periods. Yet, CME speeds of up to \SI{2000}{\km\per\s} at \SI{1}{\au} were reconstructed \citep{Russell2013}. According to the CME velocity relation (\ref{eq:kpvsv_CME_dependency_function}), a CME speed of this value would lead on average to a theoretical \Kp{} of $11.2$, however, the \Kp{} scale is capped at 9o and a \Kp{} of 9o is reached on average already at a velocity of \SI{1479}{\km\per\s}, see \autoref{fig:Kp_2dhistogram_V_sws123_fit_f2}.
\begin{figure}
	\fcapside[\FBwidth]{
		\includegraphics[width=0.6\textwidth]{figures_of_mine/chapter2/Kp_2dhistogram_V_sws123_fit_f2.pdf}
	}{
		\caption{Logarithmic fit curves for CMEs (dashed line) and streams (dotted line) with their corresponding mean MAD bands (shaded areas). I expanded the curve for CMEs up to a hypothetical \Kp{} of 12. The curve for solar wind streams is cut at \SI{1100}{\km\per\s} because streams do not occur at these high velocities. The curves are the same as in Figures~\ref{fig:Kp_2dhistogram_V_sws1_fit_e} and \ref{fig:Kp_2dhistogram_V_sws23_fit_e}.}
		\label{fig:Kp_2dhistogram_V_sws123_fit_f2}
	}
\end{figure}
...
As \Kp{} is linked to the $ap$~index, it translates to the ground magnetic field disturbance at about \SI{+-50}{\degree} dipole latitudes. $ap$ can directly be expressed in \si{\nano\tesla}. If the \Kp{} to $ap$ conversion \autoref{tab:kp_to_ap_table} would be expanded above \Kp{} 9o, the absolute ground field disturbances evoked by extremely fast CMEs could be estimated.\\

% comparison and differences to existing studies
The three \Kp--velocity distribution panels in \autoref{fig:Kp_2dhistogram_V_sws_d} look very similar to those in Figures~3 and~4 by \citet{Elliott2013}. This is because I use OMNI data and the SWS list as well, however, they use a different time resolution, data processing, and time period.
More precisely, they use 3"~hour averages of the hourly OMNI data set, modify the CME-associated periods of the SWS list with additional constraints, and use data from the time period 1963--2009.\\

\citet{Elliott2013} sort the \Kp{}--velocity distribution for CMEs and streams as well. They further differentiate between compression and rarefaction regions and compare three threshold techniques to do so: by sorting for density, dynamic pressure, and velocity slope.\\

% correlation studies between solar wind and geomagnetic indices, see Akasofu1981 p.~126, table\\


% solar activity influence
For the solar wind predictive \Kp{} models, the influence of the solar activity is not considered despite its contribution of a variation of about 1.6~\Kp~units (see \autoref{sec:kp_long_term_variations}). An analysis of the solar activity's influence on the \Kp{} predictions lies outside the scope of this thesis. Nevertheless, it would be worth examining the solar cycle's influence on the relations, especially as \citet{Wing2005} note that the predictability of \Kp{} slightly scales with solar activity.
...
% Applications
The derived \Kp{} relations are integrated into the UGOE/IAG CME forecast chain, which consists of CME source region and coronal environment analysis, 3D modeling of the CME structure, CME acceleration and propagation modeling, CME arrival time and parameter prediction, and the subsequent forecast of relevant space weather effects, such as its impact on the \Kp~index and the ionospheric TEC.

Prototype \Kp{} relations were integrated into applications developed within the AFFECTS project. The following services are accessible via the \mbox{AFFECTS} website\footnote{AFFECTS website: \urlfoot{http://www.affects-fp7.eu/services/}} and contain early results from the present \Kp{} study:
\begin{itemize*}
	...
	\item The L1 Alerts are accessible via the \href{https://itunes.apple.com/au/app/affects/id893579846}{AFFECTS app for iPhone}.
\end{itemize*}
...
- for \vBz{} it is of key importance to capture the extreme values, i.e., to use high resolution solar wind data.\\
...
- averaging over 3"~hour intervals neglects short-term geoeffective features/structures in the IMF $B_\text{z}$.\\
- 3"~hour minima consider these structures and gain a significant higher correlation.\\

list principal conclusions of this study:\\
- It is confirmed that for the prediction of \Kp{} from remote velocity determination, it is beneficial to utilize separate CME and stream forecast methods.\\
