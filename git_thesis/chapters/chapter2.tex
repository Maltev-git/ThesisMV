...
% 6~day sub-period
The second solar wind plot zooms in on the latter CME -- it covers the 6"~day time period from 26~June to 2~July 2013, see \autoref{fig:example_sw_plot_CME_b_2013-6-26_6}.
\begin{figure}[htb]
	\centering
...
From the results presented here, I conclude that when correlating \vBz{} with \Kp{} it is of key importance to capture the 3"~hour minimum values, i.e., to use high resolution solar wind data, whereas when using velocity the underlying data resolution is negligible.

It is confirmed that averaging over 3"~hour intervals neglects short-term geoeffective features/structures in the IMF $B_\text{z}$. The calculation of 3"~hour minima considers these structures and gains a significant higher correlation.

Thus, the solar wind velocity is best suited for remote forecast situations, whereas \vBz{} is not, because short-term variations in $B_\text{z}$ cannot be predicted yet.

% principal conclusions
The differences in magnitude and trend of the velocity curves confirmes that for the prediction of \Kp{} from solar wind velocity, it is beneficial to utilize separate \Kp{} relations for CMEs and streams.

The extension of the CME velocity relation from \SI{1100}{\km\per\s} to \SI{1500}{\km\per\s} can be used for predicting the \Kp~index of fast CMEs, if the \Kp--velocity dependency indeed holds true for that speed regime as well.

The ground geomagnetic disturbances of the rare, extremely fast CME events with velocities above \SI{1500}{\km\per\s} could be resolved and estimated via extending the relation between \Kp{} and \textit{ap}.


As the obtained functional relations are based only on one or two aspects of the solar wind--magnetosphere coupling, they cannot compete with full-fledged solar wind coupling functions, such as the rate magnetic flux is opened at the magnetopause (\autoref{eq:newell_merging_function}), nor with current models based on artificial neural networks or \Kp{} persistence. Nevertheless, they enable empirical estimations of the mean \Kp{} impact for special forecast situations, that is, \Kp{} can directly be quantified from measured solar activity, in-situ solar wind, and remotely determined CME and stream velocities.

- for \Kp{} nowcasts via the E-field the sw data processing is of major importance\\
- average and strong geomagnetic activity is likely underestimated by the predictions from the \Kp--velocity relations\\
