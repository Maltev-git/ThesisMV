...
For the stream velocity relation, the defined \Kp{} threshold is of little use as its proper \Kp~range is below the defining geomagnetic storm value of \Kp{}~5, that is, storms cannot be predicted via the stream relation.
...
% extreme CME speeds
CMEs can be faster than the maximal velocities included in the OMNI data. Solar wind plasma spectrometers are only able to reliably measure speeds up to about \SI{1100}{\km\per\s}, leading to data gaps during these periods. Yet, CME speeds of up to \SI{2000}{\km\per\s} were observed at \SI{1}{\au} \citep{Russell2013}. According to the CME velocity relation (\ref{eq:kpvsv_CME_dependency_function}), a CME speed of this value would lead on average to a theoretical \Kp{} of 12.2, however, the \Kp{} scale is capped at 9o and a \Kp{} of 9o is reached already at a velocity of \SI{1489}{\km\per\s}, see \autoref{fig:Kp_2dhistogram_V_sws123_fit_f}.
% 2000 km/s -> 12.17~Kp
\begin{figure}
	\fcapside[\FBwidth]{
...
A 2"~month sample period of solar wind and \Kp{} data is displayed in \autoref{fig:example_sw_plot_1month_b_2013-5-1_65}. The considered period ranges from 1~May to 5~July 2013, when the solar activity cycle~24 had its maximum. The period contains a few HSSs and CMEs.\\
\begin{figure}[htb]
	\centering
...
\begin{figure}[htb]
	...
	{Solar wind parameters, official \Kp~index, and estimated \Kp{} indices for the 6~day time period from 26~June to 2~July 2013. All panels show the same parameters as in \autoref{fig:example_sw_plot_1month_b_2013-5-1_65}. The period includes the CME from 27~June 2013.}
...
% % % former outlook
For the solar wind predictive \Kp{} models, the influence of the solar activity is not considered despite its contribution of a variation of about 1.6~\Kp~units (see \autoref{sec:kp_long_term_variations}). An analysis of the solar activity's influence on the \Kp{} predictions lie outside the scope of this thesis. Nevertheless, it would be worth examining the solar cycle's influence on the relations, especially as \citet{Wing2005} note that the predictability of \Kp{} slightly scales with solar activity.

\citet{Gopalswamy2014} indicate in particular that low solar activity leads to a decreased geomagnetc effectiveness of CMEs in general. The reasons being the reduced heliospheric pressure allows for a greater expansion of CMEs, resulting in the dilution of the magnetic fields.
...
Prototype \Kp{} relations are integrated into applications developed within the AFFECTS project. The following services are accessible via the \mbox{AFFECTS} website\footnote{AFFECTS website: \urlfoot{http://www.affects-fp7.eu/services/}} and contain early results from this \Kp{} study:
\begin{itemize*}
