...
% in-situ plots
In the following, I take sample periods of solar wind in-situ data and apply the predictive models to compare their output qualitatively with the actual measured \Kp~index.
...
I calculate the \Kp{} estimate for the CME from 27~June 2013 (see also \autoref{fig:ACE_64s_v7_thesis_CME_2013-6-26_6}) and include the 3"~hour minima values of \vBz{} in the plot, see \autoref{fig:example_sw_plot_vBz_b_2013-6-26_6}.
\begin{figure}[htb]
	\centering
	...
	{Solar wind parameters, official \Kp{}~index, and estimated \Kp{} for the time period from 26~June to 2~July 2013. The solar wind parameters are the magnetic field strength, its z"~component in GSM coordinates, and the velocity from the minutely OMNI data set. I also plot the product \vBz{} with its 3"~hour minima for illustration. The official \Kp{}~index is obtained from the GFZ~Potsdam. The \Kp{} estimate is derived via Equations~(\ref{eq:kpvsvbz_dependency_function_negative}) and (\ref{eq:kpvsvbz_dependency_function_positive}). The orange band indicates the mean MAD.}
	\label{fig:example_sw_plot_vBz_b_2013-6-26_6}
	\addtocontents{lof}{\smallskip\protect\center I created the figure myself.\medskip}
\end{figure}
It can be seen that in the considered period the \Kp{} estimate traces the actual \Kp{}~index pretty well within the mean MAD band. However, deviations are found at the times of the initial shock, and the start and middle of the MC.

It becomes apparent how important the influence of the magnetic field z"~component is, when comparing the \vBz{} and velocity estimations. I plot its in-situ magnetic field strength, $B_\text{z}$, and velocity in \autoref{fig:example_sw_plot_v_b_2013-6-26_6} together with the official \Kp{}~index and the derived \Kp{} estimates.
\begin{figure}[htb]
	\centering
