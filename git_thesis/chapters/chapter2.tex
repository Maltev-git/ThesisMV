...
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		...
		{Mean \Kp{} values (+) and MAD values (dotted lines) per \vBz~interval. The error bars represent the relative data count. The logarithmic fit (dashed line) is plotted with a mean MAD band (shaded area). The splitted function \eqref{eq:log_square_fit_function} is used for the weighted fit.}
...
The general \Kp--velocity dependency in the solar wind is apparent from the tilt of its distribution, see first panel of \autoref{fig:Kp_2dhistogram_V_sws_e}.
\begin{figure}[htb]
	\centering
	\includegraphics[width=\textwidth]{figures_of_mine/chapter2/Kp_2dhistogram_V_sws_e.pdf}
	\caption[\lofimage{figures_of_mine/chapter2/Kp_2dhistogram_V_sws_e.pdf}I created the figure myself.]
	...
	\label{fig:Kp_2dhistogram_V_sws_e}
\end{figure}
The distribution is inclined to positive values but very broad, that is, in the typical solar wind velocity range it spans over more than half of the total \Kp{} range. The comparison with the filtered data shows that \Kp{} values \num{>6.0} and velocities \SI{>850}{\km\per\s} are almost always associated with CME related periods, see right and bottom panel of \autoref{fig:Kp_2dhistogram_V_sws_e}.

% Functional dependency for CME velocity
In order to determine a functional relation between \Kp{} and CME velocity, I look at the relative \Kp{} frequencies for each \SI{10}{\km\per\s} velocity interval. The relative frequencies are plotted in the bottom panel of \autoref{fig:Kp_2dhistogram_V_sws1_d}.
\begin{figure}[htb]
	\centering
	\includegraphics[width=\textwidth]{figures_of_mine/chapter2/Kp_2dhistogram_V_sws1_d.pdf}
	\caption[\lofimage{figures_of_mine/chapter2/Kp_2dhistogram_V_sws1_d.pdf}I created the figure myself.]
	{CME fraction of the \Kp--velocity distribution (same as third panel of \autoref{fig:Kp_2dhistogram_V_sws_e}) and its relative distribution per velocity interval with the mean \Kp{} values (solid line) and their MAD (dotted lines). The bin size in velocity is \SI{10}{\km\per\s} and in \Kp{} \num{0.3}.}
	\label{fig:Kp_2dhistogram_V_sws1_d}
\end{figure}
...
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		...
		{Mean \Kp{} values (+) and MAD values (dotted lines) per velocity interval for the CME fraction of the data. The error bars represent the relative data count. The logarithmic fit (dashed line) is plotted with a mean MAD band (shaded area). The function \eqref{eq:log_offset_fit_function} is used for the weighted fit.}
...
I use this shifted data and look at the relative frequencies per velocity interval in order to find a functional dependency between \Kp{} and velocity, see bottom panel of \autoref{fig:Kp_2dhistogram_V_sws23_e}.
\begin{figure}[htb]
	\centering
	\includegraphics[width=\textwidth]{figures_of_mine/chapter2/Kp_2dhistogram_V_sws23_e.pdf}
	\caption[\lofimage{figures_of_mine/chapter2/Kp_2dhistogram_V_sws23_e.pdf}I created the figure myself.]
	{Stream fraction of the \Kp--velocity distribution (similar to second panel of \autoref{fig:Kp_2dhistogram_V_sws_e}, but with the data shifted by 9~hours) and its relative distribution per velocity interval with the mean \Kp{} values (solid line) and their MAD (dotted lines). The bin size in velocity is \SI{10}{\km\per\s} and in \Kp{} \num{0.3}.}
	\label{fig:Kp_2dhistogram_V_sws23_e}
\end{figure}
...
%determine fitting function
Again, I use the logarithmic function \eqref{eq:log_offset_fit_function} for the fitting process. The resulting values of the fit parameters are \mbox{$a = \num{5.88(38)}$}, $x' = \num{2.99(49)e2}$, and $y' = \num{-3.70(29)e1}$, with the velocity in units of [\si{\km\per\s}].
%a1 = 5.88(38)
%b1 = -37.0(29)
%x1 = 299(49)
...
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		...
		{Mean \Kp{} values (+) and MAD values (dotted lines) per velocity interval for the stream fraction of the data, shifted by 9~hours. The error bars represent the relative data count. The logarithmic fit (dashed line) is plotted with a mean MAD band (shaded area). The function \eqref{eq:log_offset_fit_function} is used for the weighted fit.}
...
\begin{itemize}
	\item The functional relation for the yearly \Kp{} averages relates \Kp{} with solar activity via the SSN, see Equation \eqref{eq:kp_ssn_relation}. The \Kp{} error to it is about $0.3$, whereas seasonal variations further contribute up to $1.3$.
	\item The functional relation for enabling \Kp{} nowcasts relates \Kp{} with the solar wind electric field, see Equations~\eqref{eq:kpvsvbz_dependency_function_negative} and \eqref{eq:kpvsvbz_dependency_function_positive}. Its MAD has a mean \Kp{} size of \num{0.7}.
	\item The functional relation for enabling CME forecasts relates \Kp{} with the velocity of CME-associated flows, see Equation~\eqref{eq:kpvsv_CME_dependency_function}. Its MAD has a mean \Kp{} size of \num{1.1}.
	\item The functional relation for enabling stream forecasts relates \Kp{} with the velocity of solar wind streams, see Equation~\eqref{eq:kpvsv_stream_dependency_function}. Its MAD has a mean \Kp{} size of \num{0.7}.
\end{itemize}
