...
Version 0.371 -- last changes \ISOToday{} \thistime{}.\\
...
\begin{small}
	...
	The bottom panel shows the relative frequency distribution of the solar wind magnetic field strength with respect to solar distance and its mean value (solid line). Data from the Helios probes is shown on the right and the extrapolation of the derived model to the near-Sun region is shown on the left.
...
\begin{abstract}
	...
	Near-Earth in-situ solar wind measurements, consisting of 35~years of minutely OMNI data, are analyzed together with the time series of the planetary geomagnetic disturbance indicator \Kp{}. Correlations and functional dependencies are compiled with regard to nowcast the geomagnetic disturbance indicator \Kp{} from in-situ measurements of the solar wind electric field and to forecast them from remotely determined velocity of coronal mass ejections and solar wind streams.
	
	%solar activity
	Further, the key solar wind parameters magnetic field strength, proton velocity, density, and temperature are correlated with the sunspot number for the purpose of deriving functional dependencies with the state of the solar cycle. The solar wind data consists of 53~years from the hourly OMNI data set.
	%solar distance
	Solar wind in-situ data from the Helios~1 and~2 missions, operational in the 1970s and orbiting the Sun within the distance range \SIrange{0.29}{0.98}{\au}, are analyzed and empirical solar wind distance dependencies are derived. Additionally, in view of the near-Sun spacecraft mission Parker~Solar~Probe which launched in mid~2018, the solar wind environment is estimated down to its closest perihelion at \num{9.86}~solar radii.
	
% 	use
	The \Kp{} relations and the empirical solar wind model derived in this work provide useful tools for estimating the corresponding quantities. The \Kp{} relations are especially valuable within the scope of space weather predictions.
\end{abstract}
