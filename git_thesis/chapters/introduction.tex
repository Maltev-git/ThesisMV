...
\begin{itemize*}
	...
	\begin{figure}[htb]
		\fcapside[\FBwidth]{
			...
			{IMF clock angle ($\theta_c$) dependency of the coupling functions' relative amplitudes. The different functions include the IMF clock angle factor by \citet{Newell2007} and the half-wave rectifier (black). I added the GSM coordinate directions in the lower left for orientation. ...half-wave is -cos(theta)!!}
...
Another sophisticated prediction method relies on artificial neural networks. Models with this method rank usually highest in prediction performance.
...
% 	correlation coefficients of over 0.88 for 1 h predictions of Kp and 0.86 for 3 h ahead
	...
	\item The SWPC provides \Kp{} predictions of about half an hour ahead based on established relations between solar wind parameters and \Kp{}. They use solar wind real-time data from spacecraft at L1. The SWPC also publishes 3"~day predictions made manually by experienced forecasters that interprete WSA-Enlil simulation results. The 3"~Day Geomagnetic Forecast website:\\
...
Coronal holes may grow or shrink to some extent during one solar rotation. However, they usually exist over many rotations and thus lead to recurrent HSSs and CIRs. This steadyness is utilized to forecast the solar wind one solar rotation in advance, simply by shifting the in-situ measurements by 27~days. The prediction accuracy can be further enhanced by shortening this period to a few days using in-situ data from spacecraft trailing the Earth in its orbit around the Sun. This is possible with the STEREO spacecraft and with a potential mission located at the fifth Lagrange point (L5). This method is applied together with an analyis of CH sizes in the STEREO+CH persistence model \citep{Temmer2018} developed at the University of Graz, a 4"~day real-time forecast is hosted on their website\footnote{STEREO+CH website: \urlfoot{http://swe.uni-graz.at/index.php/services/solar-wind-forecast-stereo-ch}}.

% ENLIL
Another prediction approach is the numerical MHD simulation of the solar wind flow from the Sun to Earth, using remote observations of the Sun and its corona. This allows forecast times of 3~to 4~days, the solar wind's average travel time to \SI{1}{\au}. The WSA-Enlil model achieves this in two steps \citep{Pizzo2011}. First, observations of the solar surface magnetic field are used to approximate the expansion of the corona and hence get the outflow at the source surface. Second, a steady solar wind flow is propagated outwards to Earth. WSA-Enlil real-time model results are provided by NOAA's SWPC\footnote{WSA-Enlil Solar Wind Prediction website: \urlfoot{https://www.swpc.noaa.gov/products/wsa-enlil-solar-wind-prediction}}.

Precise solar wind predictions will be very difficult to achieve, as it is already hard to trace solar wind measurements made at \SI{1}{\au} unambiguously back to its coronal source \citep{Cranmer2017}. It is not only complicated by stream-stream interactions but by internal solar wind processes, such as turbulence, as well.
...
It is clear that geomagnetic storms can directly be attributed to the sub-structures of CMEs. The shock causes a sudden commencement, though the interior structures create geomagnetic storms if carrying a significant southward IMF. \citet{Tsurutani1988} showed that the compressed solar wind field in the sheath region behind the shock and the magnetic flux rope field are of equal importance for the generation of geomagnetic storms. This is the reason why the geomagnetic storms' intensity and duration cannot be predicted only from remote solar observations of active regions.\\
...
A plasma spectrometer monitors the energy spectrum of the detected flow of charged particles. These plasma detectors detect the energy spectra and directions of the bulk particle flows. The energy spectra are used to further determine the bulk velocity, temperature, and density.
...
A plasma ion detector only provides an energy spectrum -- the velocity, density, and temperature of the proton and alpha ions have to be derived from it. Both ion populations can overlap under extremely hot conditions \citep{Bame1992}. The individual frequency distribution's peak energy indicates the bulk velocity, its area shows the number density, and its width scales with the temperature. I prepared a synthetic ion energy spectrum for illustration in \autoref{fig:ion_energy_spectrum_plot}. The experiment calculates these solar wind properties by numerical integration over specified energy ranges. The automatism has its limits (e.g., very low temperatures cannot be assessed correctly), however a manual fit of two superimposed Maxwell-Boltzmann curves to the energy distributions can overcome this \citep{Bame1992}.
