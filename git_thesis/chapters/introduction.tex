...
In order to quantify geomagnetic activity, it is necessary to find solar wind coupling functions that correlate well with geomagnetic indices. These relations can be used to connect solar wind parameters with geomagnetic indices via an empirically fitted function.
...
% merging functions
In general, the rate magnetic flux is opened at the magnetopause depends on the following independent factors \citep{Newell2007}:
\begin{itemize*}
...
Several different solar wind coupling functions were introduced in order to estimate the magnetic flux opening rate.

The most prominent way to quantify the reconnection rate is the induced electrical field $\vect{E}$. It represents the rate at which southward magnetic flux is transported to the magnetosphere \citep[p.~103]{Russell2007}:
\begin{align}
	|\vect{E}| = - v_\text{x} \cdot B_\text{z}	\,.
\end{align}
See the Appendix \autoref{sec:electrical_field_at_the_magnetopause} for how this relation is derived.\\


Here I list and discuss only a few of the most used merging coupling functions. There exist a lot more based on combinations of the same physical quantities.
\begin{itemize*}
	\item $v$, the solar wind velocity (cite?).
	\item $B$, the IMF strength (cite?).
	\item $E_\text{y}$, the sw? electrical field, represented by \vBz{} (cite?).
	\item The half-wave rectifier, Bs or vBs?, figure? (cite?).
	\item The Newell relation \citep{Newell2007}.
	\item The Russell-McPherron effect  (cite?).
\end{itemize*}


% viscous functions
It was found that coupling functions have the highest correlation, if they consist of a merging term combined with a viscous term \citep{Newell2008}. As the viscous term describes another kind of interaction process, it is independent from the reconnection term. It usually depends on the solar wind plasma quantities velocity and density.\\

\citet{Newell2008} deduced that the best combination is $\text{d}\Phi / \text{d}t$ together with $n^{1/2} \cdot v^2$.\\
...
the simplest coupling function between the solar wind and the Earth magnetosphere is the half wave rectifier. It is controlled by the component of the interplanetary field perpendicular to the solar wind flow and its clock angle around the flow.\\
...
\begin{align}
	|\vect{E}| = -v_\text{x} \cdot B_\text{z}\,.	\label{eq:coupling_vxBz}
\end{align}
...
half-wave rectifier, ref to R-M effect...\\
