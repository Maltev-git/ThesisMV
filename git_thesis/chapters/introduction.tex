...
%Cranmer2005:
at least for open coronal regions (CHs) MBPs are the photospheric source of the HMF in the fast sw\\
At the photosphere magnetic bright points (MBPs) are identified as intergranular magnetic flux tubes with field strenghts of \SIrange{100}{200}{\milli\tesla} \citep{Cranmer2005}.\\
Their stochastic convective appearance and motions result in wavelike fluctuations of the thin flux tubes.\\
leading to incompressible waves leak into the solar wind\\
Alfvén waves?\\
read Cranmer2005 page3 again and connect to alfvenic waves...\\

for more details about the Alfvén waves and Alfvén speed see \autoref{sec:alfvén_waves}\\

low chromosphere:\\
thin flux tubes expand laterally and merge to a homogeneous network field\\

below the chromosphere-corona transition region:\\
the network magnetic field expands laterally and merges again to a large-scale canopy (image)\\

near-Sun field configuration:\\
``above the Alfvén critical point -- where the wind speed equals the local Alfvén speed at about \SI{10}{\Rs} -- both the inward and outward modes are advected (advect: convect horizontally) outward with the wind\\
...
\begin{figure}[htb]
	\begin{floatrow}
		\ffigbox{
			...
			\caption{Solar magnetic field geometry from the DQCS model with field lines (solid) and constant field strength surfaces (dashed). The quadrupole part allows equatorial outflows along the current sheet. Credit: \citet[][Fig.~3]{Banaszkiewicz1998}, reproduced with permission, \textcopyright~ESO.}
