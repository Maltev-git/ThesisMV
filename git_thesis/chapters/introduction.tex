...
Space weather is the field of study that comprises all dynamic effects generated by the Sun in its domain of influence -- the heliosphere. Naturally, it is focused especially on the solar effects observed on Earth and its local environment -- the magnetosphere. Space weather deals with effects on time scales similar to the terrestrial weather, that is, days and weeks.\\

Most of its effects are introduced through the Sun's impact on the magnetosphere, ionosphere, or upper atmosphere (correct? LEO drag due to flare heating?).



solar wind affects magnetosphere and causes:

solar wind changes IMF and causes:
Forbush decreases

solar radiation affects ionosphere and causes:

solar radiation affects upper atmosphere and causes:
...
- Notable direct risks for humans are enhanced radiation doses during polar flights and especially for humans in orbit outside the protective atmosphere.
...
Space weather becomes more and more important, not only because of the increasing use of sensitive technology at Earth, but also because of the anticipated expansion of interplanetary space activities in the near future.\\
...
Bothmer2007book: ``Space weather can be understood as a field of research that will provide new insights into the complex influences and effects of the Sun and other cosmic sources on interplanetary space, the Earth's magnetosphere, ionosphere, and thermosphere, on space- and ground-based technological systems, and beyond that, on their endangering effects to life and health.''\\
...
Even in ``ancient'' times (when?) a correlation between solar particles and disturbances in the magnetosphere were known of \citep{Bartels1962}.\\
...
seasonal effects from Earth orbital distance, inclination (solar rotation axis angle) and Earth tilt; make geometry fig?\\

% flare impact and ionosphere
``Flares are one of the three major components of space weather – next to coronal mass ejections (CMEs) and solar energetic particles (SEPs) – and pose significant threats to satellites, radio communication, and navigation by Global Navigation Satellite Systems.'' from Kraaikamp2015\\
Kraaikamp2015: \url{https://www.swsc-journal.org/articles/swsc/pdf/2015/01/swsc140062.pdf}\\

X"~ray fluxes from flares are also responsible for producing sudden, short-lived enhancements in the electron content of the ionosphere (TEC), known as sudden ionospheric disturbances. cite from \citet{Gosling1993}\\

ionospheric variations due to solar radiation (day/night cycle and flares)\\
ionosphere -> TEC -> GNSS error\\

see description in SWPC flares: \url{https://www.swpc.noaa.gov/products/goes-x-ray-flux}\\
...
influences on human infrastructure/technical systems/animals, such as birds and whales; whales: \citep{Vanselow2017}\\
