...
% CME in-situ plot
Three CMEs can be seen in the solar-wind in-situ measurements showed previously in \autoref{fig:ACE_64s_v7_thesis_CIRs_2013-5-1_65_plot}, passing by the ACE spacecraft at L1 on 24~May, 6~June, and 27~June in 2013. The latter CME is presented in detail in \autoref{fig:ACE_64s_v7_thesis_CME_2013-6-26_6_plot}, it has a well-structured MC. In addition to showing the solar-wind in-situ parameters, I indicated the shock, compression zone and the MC with dotted lines, and plotted the geomagnetic \Kp{}~index in order to visualize the CME's impact on the magnetosphere. This MC contains a sector boundary and is trailed by an interaction region caused by a following HSS.
\begin{figure}[htb]
	\centering
...
The CME described before is indeed coming from the northern hemisphere, as can be seen from the STEREO~A coronagraph image in \autoref{fig:STEREO_A_COR2_20130624_022400_dbc2A} three days earlier and from its position in relation to Earth at that time, see \autoref{fig:STEREO_positions_w_269707595}.
\begin{figure}[htb]
	\begin{floatrow}
		\ffigbox{
...
Apparently, only the lower part of the CME grazes the ecliptic and arrives at Earth.\\
...
see \autoref{fig:Cremades2004_fig15}\\
\begin{figure}[htb]
	\centering
	...
	{CME projection scheme for frontside events. Note that for events originating from the backside panel a) has to be mirrored vertically. Credit: {\citet[Fig.~15]{Cremades2004}}, \textcopyright~ESO, permission...}
