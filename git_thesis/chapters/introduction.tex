...
From the source surface on, the magnetic field -- frozen within the plasma -- is carried by the solar wind radially outwards into the heliosphere.\\

the superradial field expansion up to the source surface can be seen in fig...\\

In solar minimum conditions, with polar coronal holes and closed equatorial fields, an equatorial current sheet exists, separating both hemispheres.\\

The DQCS model with its quadrupole part having a direct link from the solar equator to the equatorial plane and current sheet. (see \autoref{fig:Banaszkiewicz1998_DQCS_model_raw}) \citep{Banaszkiewicz1998}\\
The dipole plus quadrupole plus current sheet (DQCS) model constructed by \citet{Banaszkiewicz1998}

analytical solar magnetic field model for solar minimum conditions was\\
...
\begin{figure}[htb]
	\begin{floatrow}
		\ffigbox{
			...
			\caption{Solar magnetic field geometry in the polar plane for solar cycle minimum. Magnetic field lines (solid) and constant field strength surfaces (dashed) from the DQCS model are plotted. The model's quadrupole part allows equatorial outflows along the current sheet. Credit: \citet[][Fig.~3]{Banaszkiewicz1998}, reproduced with permission, \textcopyright~ESO.}
			\label{fig:Banaszkiewicz1998_DQCS_model_raw}
		}
		\ffigbox{
			\includegraphics[width=\Xhsize]{images/Owens2013_PFSS_Sectors_screenshot.png}
		}{
			\caption{Illustration of the solar magnetic field Parker spiral formation by rotation of the solar-wind source surface. Between solar-wind flows of opposite magnetic field polarity a heliospheric current sheet (HCS) forms. Credit: \citet[Fig.~1]{Owens2013}, adapted from \citet[Fig.~1]{Schatten1969}.}
