...
\begin{figure}[htb]
	\begin{floatrow}
		\ffigbox{
			...
			{Image of the solar corona out to \SI{15}{\Rs} from 23~September 2012 taken by the SECCHI/COR2 coronagraph onboard the STEREO~A spacecraft. The solar disk is covered by the occulter disk and its position is indicated by the white circle. The bright blob to the top right is the CME; the smooth elongated radial lines are solar wind streamers. Courtesy of \href{https://stereo.gsfc.nasa.gov/gallery/copyright.shtml}{STEREO/COR2 consortium (NASA)}. ...find event with shock and flux rope?}
...
This white-light CME projection scheme as well as the BSS for MCs are based on the notion that the orientation of flux ropes propagating outwards generally stays aligned with the neutral polarity inversion line on the solar surface where the filament erupted \citep{Marubashi1997,Bothmer1998}. In fact, it is shown that flux ropes are not likely to rotate after their initiation but maintain the orientation of their main axis parallel to the magnetic neutral line even in interplanetary space \citep{Marubashi2015}.
...
SEPs often accompany CMEs, radio bursts...\\
...
NOAA: solar energetic particle (SEPs)\\
by CMEs or flares accelerated charged protons in corona\\
protons that have relativistic energies; cover distance to Earth within half an hour\\
...
typical MC-plasma parameter values\\

3-part structure figure...\\
abbreviate to MFR?\\

implement Cranmer2017...\\
...
Space weather is the field of research that comprises all dynamic effects generated by the Sun in its domain of influence -- the heliosphere. Naturally, it is focused especially on the solar effects observed on Earth and in its local environment, the ionosphere and the magnetosphere. Solar events affect sensitive technical systems and strong events are even able to disrupt them and pose a threat to humans \citep{Bothmer2007}. Space weather deals with effects on time scales similar to the terrestrial weather, that is, days and weeks.\\

The existence of a direct solar-terrestrial relation was known of early on. \citet{Carrington1859} made the first connection between solar flares and disturbances in the terrestrial magnetic field when he observed a brightening on the solar disk on 1~September 1859 and suggested its connection to a strong geomagnetic storm that occurred about 17~hours later. Several categories of solar-terrestrial correlations were already listed by \citet{Bartels1962}: Events initiated by irregular solar X"~ray flares and CMEs, by the 11"~year solar activity cycle, and by the periodic 27"~day solar rotation. Other time related variations include seasonal effects caused by the Earth's orbital distance change over the year, the inclination of the Sun's and Earth's rotation axes to the ecliptic normal, and the terrestrial magnetic field's dipole axis tilt to the rotation axis. There are also the daily effects from the Earth's rotation.\\

The major driving forces behind severe space weather events are CMEs, solar X"~ray flares, and solar energetic particle (SEPs) events. Most of the terrestrial space weather effects are introduced through their influence on the magnetosphere, ionosphere, and upper atmosphere. Space weather events are commonly classified into geomagnetic storms, solar radiation storms, and radio blackouts\footnote{NOAA Space Weather Scales website: \urlfoot{https://www.swpc.noaa.gov/noaa-scales-explanation}}.\\

The solar wind's internal structures affect the magnetosphere and can cause disturbances in its magnetic field, that is, geomagnetic storms and substorms, which are described in more detail in the following sections. Notable consequences of disturbances in the terrestrial magnetic field are listed in the following:
\begin{itemize*}
	\item Impact on the navigation and behaviour of animals, such as migratory birds (cite?) and whales \citep{Vanselow2017}.
	\item Stronger aurora and shifting of the auroral oval to lower latitudes.
	\item Direct risks for humans from enhanced radiation doses during polar flights and especially for humans in orbit outside the protective atmosphere.
	\item Geomagnetic storms heat the upper atmosphere, causing it to expand, and generate an increased drag for satellites in low Earth orbits (NOAA cite).\\
\end{itemize*}

Solar EUV and X"~ray radiation affect the ionosphere apart from their regular day/night influence. Solar flares can create sudden ionospheric disturbances \citep{Gosling1993}, that is, abrupt enhancements in the ionospheric total electron content (TEC). These TEC variations can threaten the following technical systems \citep{Kraaikamp2015}:
\begin{itemize*}
	...
	\item Induced changes in the stratification (D-, E-, and F"~layers) of the ionosphere affect radio communication and lead to outtages in different frequencies.
\end{itemize*}

High fluxes of SEPs induce solar radiation stormswith durations of ranging from hours to days that have the following effects:
\begin{itemize*}
	\item Damage to electronic circuits of satellites.
	\item Damage to biological DNA of humans in space.
	\item The SEPs are guided by the terrestrial magnetic field down to the polar regions. Humans flying in aircraft near polar latitudes are exposed to higher radiation during strong radiation storms.
...
The cosmic radiation level at Earth is modulated by solar activity through the changes in the overall HMF. Even the passing of strong MCs lead to reductions in cosmic radiation, called Forbush decreases.\\

Most space weather effects are connected with economic interests and some impacts on technical systems can even influence human lives. This creates a strong interest in forecasting the space weather. Space weather is a relatively new field of study, however, it becomes more and more important, not only because of the increasing use of sensitive technology at Earth, but also because of the anticipated expansion of interplanetary space activities within the near future. There exist lots of dedicated institutions and a multitude of forecasting techniques for the different aspects of space weather and more are being built.\\

Predictions of magnetospheric disturbances are based on knowledge of the solar wind properties, which the magnetosphere will encounter. Thus in a first step, forecast methods have to predict the solar wind at Earth in advance and derive its expected impact on the magnetosphere in a second step. The first part of this study contributes to the latter step with predicting the amplitude of magnetospheric disturbances caused by solar wind. The basic structure of the magnetosphere, its coupling to the solar wind, and how disturbances are quantified/measured is described in the following sections. Existing forecast methods for geomagnetic disturbances and their solar wind sources are described in \autoref{sec:forecast_methods}.\\


----------------\\

Notable impacts on technical systems are increased GNSS positioning errors, disturbances in satellite communications and radio signals, power grid outtages, increased corrosion of long conducting infrastructure, such as oil pipelines, rail tracks, and power lines, increased satellite drag in low Earth orbits, faulting satellite electronics.\\


Notable effects on the rest of the solar system bodies:
\begin{itemize*}
	\item stripping of cometary tails
	\item enhanced atmospheric losses
	\item impacts on other planetary magnetospheres
	\item modulated cosmic radiation
	\item changing shape and size of the heliosphere
	\item episodes of enhanced radiation
\end{itemize*}


- In the near future, direct space weather impact on humans on space missions outside the protective magnetosphere (Moon, Mars).\\
...
enhanced Van~Allen radiation belt intensities during geomagnetic storms\\

magnetopause front distance; under extreme SW conditions GEO satellite crossings at \SI{6.6}{\RE}\\
...
The geomagnetic field's strength decreases while it extends out into space. Its influence ends where its magnetic pressure balances the ram pressure of the solar wind. This outer boundary is the magnetopause, in average it has a sunward distance of about \SI{8.2}{\RE}. In response to the continuous changes in the solar wind pressure, the magnetopause usually is in a state of fast radial inward-outward motion and its displacement can reach several \si{\RE} \citep{DeKeyser2005}. The solar wind flow shapes the magnetopause into a surface of teardrop form, named magnetosphere by \citet{Gold1959}, whose so-called magnetotail length varies around \SI{100}{\RE} and points away from the Sun.\\

The solar wind compression at the front of the magnetopause creates a bow shock, whose standoff distance to the magnetopause is a few \si{\RE} and which varies largely under extreme solar wind conditions (cite?). The shocked solar wind plasma flows within the magnetosheath around the magnetosphere, as seen in \autoref{fig:Davies1990_magnetosphere_sharpened}.\\
\begin{figure}[htb]
	\fcapside[\FBwidth]{
...
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		...
		{Schema of the magnetopause's inner 3D structure with focus on its current systems. The directions of the magnetic field lines and of the electric currents are indicated with blue and yellow arrows. Credit: {\citet[Fig.~2.12]{DeKeyser2005}}, adapted from {\citet[Fig.~1.18]{Kivelson1995}}, \textcopyright~Springer, reproduced with permission.}
...
There exist different mechanisms how solar wind couples to the magnetosphere and deposits energy into it.\\

Solar wind interaction processes with the magnetosphere\\
there are several underlying physical mechanisms\\
ways for solar wind momentum and energy transfer into magnetosphere:\\
- reconnection\\
- compression\\
- KH turbulence; waves/eddies\\
- induction\\
- sw entering sphere; influx of solar wind plasma\\
...
\begin{itemize*}
	...
	\item turbulence
	\item induction
	\item solar wind influx
\end{itemize*}


\citep{Phan2005}:\\
In its simplest description, the magnetopause is an impenetrable boundary that separates the solar wind from Earth’s magnetosphere. The well-established existence of plasma of dominantly solar wind origin inside the magnetosphere does, however, imply that solar wind plasma is able to penetrate the magnetopause. Establishing the mechanisms by which this occurs is one of the greatest challenges of magnetospheric physics. Candidate processes include magnetic reconnection, the Kelvin-Helmholtz (KH) instability, diffusion, and impulsive penetration.\\
...
reconnection X"~line (the line along which oppositely directed magnetic field lines reconnect); add figure...\\
...
during southern IMF\\
figure...\\
Where the terrestrial magnetic field and the IMF come into antiparallel contact, the magnetosphere opens up to the HMF and reconnection of the field lines occurs. At the reconnection location an X"~line exists that accelerates jets of particles. Its location on the magnetopause shifts depending on the IMF direction.\\
...
% [Magnetopause reconnection (Bz, X-line)]
...
see \autoref{fig:NASA_reconnection}\\
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		...
		{Schema of a reconnection region. The dashed lines represent magnetic field lines. The red crosses indicate the MMS spacecraft. change figure?... Credit: \href{https://mms.gsfc.nasa.gov/science.html}{NASA/GSFC Magnetospheric Multiscale (MMS) mission}.}
