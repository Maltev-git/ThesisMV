...
\begin{itemize*}
	...
	\begin{figure}[htb]
		\fcapside[\FBwidth]{
			...
			{IMF clock angle ($\theta_c$) dependency of the coupling functions' relative amplitudes. The different coupling functions include the IMF clock angle factor by \citet{Newell2007} and the half-wave rectifier (black). I added the GSM coordinate directions in the lower left for orientation.}
...
Another sophisticated prediction method relies on artificial neural networks. These models rank highest in prediction performance.

Here a few of the currently existing \Kp{} prediction models are mentioned, including now- and forecasts:
\begin{itemize*}
	...
	NASA/SWRC (CCMC); see Savani2017\\
	
	\item The $\text{d}\Phi / \text{d}t$ coupling relation is being implemented into CME forecast procedures \citep{Savani2017}.
	
	\item \citet{Elliott2013}: The \Kp~index and solar wind speed relationship: Insights for improving space weather forecasts
	
	\item Aleksei's f/c model; Ukraine; developed within AFFECTS project
