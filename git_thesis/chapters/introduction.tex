...
Flares and CME shock fronts accelerate coronal particles, producing SEPs. These are guided by the IMF along Parker spiral through the heliosphere. High fluxes of SEPs induce solar radiation storms with durations ranging from hours to days that have the following effects \citep{Bothmer2007}:
\begin{itemize*}
...
\begin{itemize*}
	...
	\item The average solar wind pressure and HMF scale with solar activity and therefore modulate the shape and size of the heliosphere. As cosmic rays are influenced by the HMF, the local cosmic ray radiation level scales inversely with solar activity. Even the passing of strong shocks and MCs lead to reductions in local cosmic ray radiation, which are called Forbush decreases.
...
% Kp index
However, for the assessment of the planetary geomagnetic activity, another index has been more widely used than $Dst$, the \Kp~index \citep{Gonzalez1994}. \Kp{} is the most prevalent index used by space weather forecasters to indicate the severity of geomagnetic storms \citep{Wing2005}, for instance, \Kp{} is the basis for NOAA's G"~Scale\footnote{NOAA Space Weather Scales website: \urlfoot{https://www.swpc.noaa.gov/noaa-scales-explanation}}.
...
The timing and intensity of geomagnetic storms is sought to be forecasted from knowledge about the upcoming solar wind conditions. This means that there are two questions to this task:

First, how does the incoming magnetized solar wind plasma result in magnetospheric disturbances? Therefore, as the \Kp~index is the measure for the size of geomagnetic disturbances, a function relating \Kp{} to solar wind parameters is essential.

Second, what state will the arriving solar wind be in at the magnetopause? Hence, a forecast using remote observations is required which is able to predict the arrival time and conditions of solar wind streams and CMEs. Both questions are addressed in the following two sections.
...
The major interaction mechanisms between solar wind and magnetosphere are magnetic reconnection and viscous turbulence, as mentioned previously in \autoref{sec:solar_wind_coupling_mechanisms}.

% merging functions
According to \citet{Newell2007}, the rate magnetic flux is opened at the magnetopause generally depends on the following independent factors:
\begin{itemize*}
	...
	\item The length of the reconnection X"~line on the magnetopause.
\end{itemize*}

Many solar wind coupling functions with differing complexity were proposed in order to characterize the interaction process with the magnetosphere. There exist a lot of functions based on combinations of the same physical quantities, often including the IMF direction \citep{Newell2007,Lockwood2013}. Here I list and discuss only a few of the most used merging coupling functions that estimate the magnetic flux opening rate.
\begin{itemize*}
	...
	\item $B_\text{s}$, the half-wave rectifier as defined by \citep{Russell1973}, see also the R-M effect in \autoref{sec:russell_mcpherron_effect}. It describes a simple interaction in GSM coordinates that is proportional to the southward IMF component and zero during northward IMF:
	\begin{align}
		B_\text{s} =
		\begin{cases}
	...
	\item $|\vect{B}| \sin^n\left(\theta_c / 2\right)$, a term depending on the IMF clock angle $\theta_c$ and in shape similar to the half-wave rectifier. However, it is continuous around zero and therefore often preferred over $B_\text{s}$ \citep{Lockwood2013}. The IMF clock angle is defined as $\theta_c = \tan^{-1}\left(B_\text{y} / B_\text{z}\right)$ and the exponent is often chosen as $n = 2$ or $n = 4$.
	
	\item $\vect{E}$, the solar wind electrical field -- with some assumptions often reduced to $E_\text{y}$. It is one of the most prominent coupling functions and represents the rate at which southward magnetic flux is transported to the magnetosphere \citep[p.~103]{Russell2007}:
	\begin{align}
		\vect{E} &= -\vect{v} \times \vect{B}	\,,\\
		|\vect{E}| &= E_\text{y} = -v_\text{x} B_\text{z}	\,.	\label{eq:coupling_vxBz}
	\end{align}
	How this relation is derived and which assumptions are made for the reduction to $E_\text{y}$ is described in the Appendix \autoref{sec:electrical_field_at_the_magnetopause}.
	
	\item $\text{d}\Phi / \text{d}t$, a relation derived by \citet{Newell2007} that empirically represents the rate magnetic flux is opened at the magnetopause. It shows the highest correlation with multiple geomagnetic indices out of various coupling functions.
...
\citep{Lockwood2013}\\
% https://link.springer.com/content/pdf/10.12942%2Flrsp-2013-4.pdf
The power density in the solar wind at Earth is dominated by the kinetic energy of the bulk flow of the particles and so the square of the solar wind velocity is another important factor. Hence a simple “coupling function”, designed to quantify the effect of the solar wind on geomagnetic activity, is $B_\text{s} \cdot v^2$.\\
the epsilon parameter, based on the energy density in the solar wind hitting the Earth’s space environment\\
P-alpha, which is the power coupled into the magnetosphere. It is the product of the power density in the solar wind, times the cross sectional area of the magnetosphere presented to the solar wind, times the fraction of the incident power that crosses the magnetopause.\\




% viscous functions
It was found that coupling functions have the highest correlation, if they consist of a merging term combined with a viscous term \citep{Newell2008}. As the viscous term describes another kind of interaction process, that is, reconnections due to Kelvin-Helmholtz instabilies at the flanks of the magnetopause, it is independent from the dayside reconnection term. It usually depends on the solar wind plasma quantities velocity and density.\\

\citet{Newell2008} deduced that the best combination is $\text{d}\Phi / \text{d}t$ together with $\sqrt{n} \cdot v^2$.\\

In order to relate the coupling functions to geomagnetic indices, relations are obtained by empirically fitting both time series. This thesis adds more empirical relations to the \Kp~index -- for the cases when only $v$ or \vBz{} are known.\\
...
IMF field lines impacting the magnetopause merge with the probability depending on the IMF clock angle; this factor is empirical \citep{Newell2007}\\
...
fraction of field lines which merge\\
half-wave rectifier compared to Newell's IMF clock angle factor, see \autoref{fig:coupling_angle_b}\\
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		\includegraphics[width=0.6\textwidth]{figures_of_mine/gnuplots/coupling_angle_b.pdf}
	}{
		\caption[\lofimage{figures_of_mine/gnuplots/coupling_angle_b.png}]
		{IMF clock angle ($\theta_c$) dependency of the coupling functions' relative amplitudes. The different coupling functions include the half-wave rectifier (black). I plotted the GSM coordinate directions in the upper left. add (Newell et al.) and remove example arrow and change colors...}
		\label{fig:coupling_angle_b}
	}
\end{figure}
...
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		...
		{Sketch indicating where reconnection of magnetic field lines on the magnetopause occurs, for the cases of northward (upper panel) and southward (lower panel) IMF. The central dot indicates the position of Earth and the arrowed lines the direction of the magnetic field. The locations of X"~line reconnection are denoted with N and the short arrows indicate the movement of the field lines. Credit: {\citet[p.~116, Fig.~4.8]{Russell2007}}, \textcopyright~Praxis Publishing, get permission..., adapted from {\citet{Dungey1961,Dungey1963}}.}
...
include reconnection location figure?\\
...
regions of antiparallel magnetic fields on the magnetosphere, see \autoref{fig:Russell2007_fig4_10_VBbook_magnetopause_reconnection_regions}\\
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		\includegraphics[width=0.6\textwidth]{figures_of_others/images/Russell2007_fig4_10_VBbook_magnetopause_reconnection_regions.png}
	}{
		\caption[\lofimage{figures_of_others/images/Russell2007_fig4_10_VBbook_magnetopause_reconnection_regions.png}]
		{Regions of antiparallel magnetic fields on the magnetosphere for different IMF directions derived from a simulation. Credit: {\citet[Fig.~4.10]{Russell2007}}, \textcopyright~Praxis Publishing, get permission..., after {\citet[Fig.~2]{Luhmann1984}}.}
		\label{fig:Russell2007_fig4_10_VBbook_magnetopause_reconnection_regions}
	}
\end{figure}
