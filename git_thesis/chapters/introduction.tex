...
Observations, made during sounding rocket flights, showed that the fast solar wind emerges from extended areas of reduced soft X-ray emission, subsequently called coronal holes (CHs) \citep{Krieger1973,Hundhausen1977}. The magnetic field polarities found in CHs are associated with the magnetic field directions observed in solar wind, see \autoref{fig:Hundhausen1977_fig10}.\\
\begin{figure}[htb]
	\fcapside[\FBwidth]{
...
The coronal plasma escapes CHs, following the open magnetic field lines outwards into space.\\
% CHs emit streams of high speed solar wind.\\
% fast wind emerges from CHs\\

%sources of slow wind
In regions with closed field lines, the coronal plasma is trapped. 
slow wind comes from closed field streamers\\

different types of slow solar wind \citep{Schwenn1983}\\

It is found to be difficult to trace the slow solar wind types to different origins and distinguish between them, because most parameters are highly variable in time. Differentiating tendencies are seen for some indicators \citep{Kilpua2016}:\\
tracing slow solar wind sources with:\\
elemental ion ratios (e.g., He/H ratio)\\
heavy ion charge states (e.g., $\text{C}^{+6}\!\!/\text{C}^{+4}$, $\text{O}^{+7}\!/\text{O}^{+6}$)\\
carbon charge states are the good indicators of the coronal temperature and in particular the $\text{C}^{+6}\!/\text{C}^{+4}$ ratio is sensitive to the solar wind type (Landi2012).\\
specific entropy [$\ln(T/n^{\gamma-1})$] has been shown to differentiate between the solar wind from different source regions\\



there are different sources possible:\\
helmet streamers from loop cusps, magnetic sector boundaries, superradial expansion, \\
...
based on the inspection of solar wind\\
