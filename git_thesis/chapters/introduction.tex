...
The major driving forces behind space weather are CMEs, solar X"~ray flares, and solar energetic particle (SEPs) events. Most of the terrestrial space weather effects are introduced through the their influence on the magnetosphere, ionosphere, and upper atmosphere.\\

The solar wind's internal structures affect the magnetosphere and can cause disturbances in its magnetic field, that is, geomagnetic storms and substorms, which are described in more detail in the following sections. Notable consequences of changes/disturbances in the terrestrial magnetic field are listed in the following:
\begin{itemize*}
	\item Impact on the navigation and behaviour of animals, such as migratory birds (cite?) and whales \citep{Vanselow2017}.
	\item Stronger aurorae even at lower latitudes.
...
Solar EUV and X"~ray radiation affect the ionosphere apart from their regular day/night influence. Solar flares can create sudden ionospheric disturbances, that is, abrupt enhancements in the ionospheric total electron content (TEC) \citep{Gosling1993}. These TEC variations can threaten the following technical systems \citet{Kraaikamp2015}:
\begin{itemize*}
...
solar energetic particles cause:
faulting satellite electronics
...
Notable space weather effects are:\\
- In the near future, direct solar wind impact on humans on space missions outside the protective magnetosphere (Moon, Mars).
...
The field's strength decreases while it extends out into space. The influence of the geomagnetic field ends at the magnetopause (or bow shock?), where its pressure balances that of the solar wind pressure.\\

The solar wind flow shapes the geomagnetic field into a teardrop form, called magnetosphere (Gold...). The solar wind compression at the front of the magnetopause creates a bow shock, whose distance to Earth is in average around XX~Re, but varies largely under extreme solar wind conditions (cite?). Its so-called magnetotail has a size of XX~Re and points away from the Sun (cite?).\\
...
bow shock, magnetotail, magnetosheath, magnetopause, see \autoref{fig:Davies1990_magnetosphere_sharpened}\\
\begin{figure}[htb]
	\centering
...
% magnetopause front distance
magnetopause front distance: the dynamic ram pressure from the solar wind is equal to the magnetic pressure from the Earth's magnetic field. about \SI{10}{\RE}; under extreme SW conditions geosynchronous Earth orbit (GEO) satellite crossings at \SI{6.6}{\RE}\\

DeKeyser2005:\\
The magnetospheric field magnitude decreases with increasing distance from Earth, but less rapidly than a dipole field in a vacuum. Indeed, the surface current induces a magnetic field that adds to the geomagnetic field inward of the magnetopause, while it prevents the Earth’s field lines from penetrating into the magnetosheath, where the field is relatively weak. Just before the magnetopause is encountered, at about \SI{8.2}{\RE}, the measured field is about twice that of the dipole (see e.g., Chapman and Bartels, 1940). In the magnetosheath, the field intensity is usually lower and has larger variability, reflecting the fluctuations in the interplanetary magnetic field.\\

% bow shock
bow shock standoff distance:	\citep[p.~112]{Bothmer2007}\\
\begin{align}
	d = 107.4 (N V^2)^{-1/6}
\end{align}
with distance in \RE{}\\
...
% tail
tail length around \SI{100}{\RE}; plasmoids...\\



DeKeyser2005:\\
the magnetopause is almost always in rapidly changing inward-outward motion with typical speeds of tens of km/s. The magnetopause does not have much mass, so its speed can change abruptly, and by large amounts, in response to even minor changes in the magnetosheath pressure. The amplitude of the radial motion of the magnetopause can be several Earth radii.\\

current layer on magnetopause\\
ring current systems\\
the magnetosphere's current system, see \autoref{fig:DeKeyser2005_magnetosphere}\\
\begin{figure}[htb]
	\centering
	...
	{Schema of the magnetopause's inner 3D structure with focus on the current system. The directions of the magnetic field lines and of the electric currents are indicated with blue and yellow arrows. Credit: {\citet[Fig.~2.12]{DeKeyser2005}}, adapted from {\citet[Fig.~1.18]{Kivelson1995}}, \textcopyright~Springer, reproduced with permission.}
