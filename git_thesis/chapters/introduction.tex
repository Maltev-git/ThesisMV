...
% auroral ovals
Reconnection at the magnetopause releases and accelerates the local plasma from the reconnection site along the magnetic field lines. Thus, this plasma reaches the field lines' polar footpoints and interacts with the particles of the ionosphere, creating the aurora. The auroral ovals are located at the boundaries between the closed terrestrial field and the field open to the IMF. The open field regions enclosed by the auroral ovals are the polar caps.\\

The Dungey cycle is not a steady-state process, that is, the rates of opened and reclosed terrestrial magnetic flux are equal only in the long term. The dayside opening rate $\Phi_\text{D}$ is modulated by the dynamic behavior of the solar wind and the orientation of the IMF, whereas the nightside closing rate $\Phi_\text{N}$ depends on the situation in the magnetotail \citep{Milan2007}, see also \autoref{fig:Milan2009_magnetosphere}.
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		...
		{Schema of the magnetosphere for the case of southern IMF direction with emphasis on open (blue lines) and closed (red lines) magnetic flux during times of southern IMF. The magnetopause is indicated by the dashed line. The opening flux rate at the dayside is denoted with $\Phi_\text{D}$ and the closing flux rate at the nightside with $\Phi_\text{N}$. The inset shows day- and nightside of Earth and the relation between auroral ovals (yellow areas) and the open flux at the polar caps ($F_\text{PC}$). The ring current intensity $H_\text{SYM}$ and direction (green arrow) are plotted as well. Credit: {\citet[Fig.~1]{Milan2009}}, \textcopyright~American Geophysical Union, reproduced with permission.}
		\label{fig:Milan2009_magnetosphere}
	}
\end{figure}
The propagation time of the solar wind from the sunward magnetopause to the magnetotail amounts to a lag of about half an hour. Thus, the total amount of open flux $F_\text{PC}$ varies with time so that the ionospheric polar caps expand/contract with time as well \citep{Siscoe1985}:
\begin{align}
...
% tail reconnection
The reconnection in the magnetotail is not steady but occurs in regular intermittent reconnection bursts. These produce plasmoids that release from the tail and are swept away with the solar wind. The tail reconnection events create sudden brightenings and increased activity in the aurora. These events are called substorms and they occur frequently every few hours.\\
...
[40 ] During periods of southward IMF the polar cap expands at a rate consistent with reconnection along an effective dayside magnetopause merging line of approximately 5 RE in length. During the 73 hours of observations the polar cap was not observed to exceed 0.9 GWb in flux or decrease below 0.2 GWb. Above polar cap fluxes of 0.5 GWb the occurrence of tail reconnection is greatly  enhanced, which subsequently leads to a reduction of the open flux. We interpret our observations as providing strong supporting evidence for the expanding/contracting polar cap paradigm. In addition, we find no evidence for a trickle of nightside reconnection outside of the major events, making substorms the major nightside flux closure process.\\
...
difference to geomagnetic storms\\
...
% wiki ring current:
The particles of the ring current produce a magnetic field in opposition to the Earth's magnetic field and so an Earthly observer observes a decrease in the magnetic field in this area. The negative deflection of the Earth's magnetic field due to the ring current is measured by the Dst index.\\
