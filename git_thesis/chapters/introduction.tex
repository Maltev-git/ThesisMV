...
\begin{itemize*}
	...
	\item Slow unidentified CMEs may contribute to slow wind observations as well.
\end{itemize*}
...
\end{figure}
For the solar backside, the neutral lines are reversed and according to the scheme so are the CME orientations -- thus the broad CME appearing behind the solar limb seen in \autoref{fig:CME_COR2_0120923_182400_dbc2A} matches this scheme. The scheme can effectively be applied for CMEs with source regions located less than \SI{50}{\degree} from the solar equator \citep{Webb2012}.
...
\begin{itemize*}
	...
	\begin{figure}[htb]
		\fcapside[\FBwidth]{
			...
			{IMF clock angle ($\theta_c$) dependency of the coupling functions' relative amplitudes. The different coupling functions include the IMF clock angle factor by \citet{Newell2007} and the half-wave rectifier (black). I added the GSM coordinate directions in the upper left for orientation.}
