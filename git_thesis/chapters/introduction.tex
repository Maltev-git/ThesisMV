...
Solar wind influences the terrestrial magnetosphere and can cause strong disturbances therein that are called geomagnetic storms. These events are able to disturb sensitive technical systems. Understanding the properties of the solar wind helps with the prediction and protection of humanity's assets.\\
...
thermosphere, on space- and ground-based technological systems, and beyond that, on their endangering effects to life and health.''\\
...
see \autoref{fig:Hundhausen1977_fig20bcd}\\
\begin{figure}[htb]
	\centering
...
see \autoref{fig:Davies1990_magnetosphere_sharpened}\\
\begin{figure}[htb]
	\centering
...
see \autoref{fig:DeKeyser2005_magnetosphere}\\
\begin{figure}[htb]
	\centering
...
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		...
		{reconnection and compression depending on the interplanetary magnetic field orientation. Credit: {\citet[p.~116, Fig.~4.8]{Bothmer2007}}, adapted from {\citet{Dungey1961,Dungey1963}}. get permission... accelerated flows are arrowed; N points are X points...}
...
Phan2005, Magnetopause Processes:\\
When the IMF changes, the reconnection site moves and flux transfer events (FTEs) (presumably due to intermittent reconnection) can be formed. The accumulated evidence suggests that the key controlling factor of the steadiness of the reconnection process is the steadiness of the solar wind and IMF. Thus intermittency does not appear to be a fundamental property of the reconnection process at the
magnetopause.\\
...
reconnection X-line (the line along which oppositely directed magnetic field lines reconnect)\\
...
Dungey cycle...\\
see \autoref{fig:Milan2009_magnetosphere}\\
\begin{figure}[htb]
	\fcapside[\FBwidth]{
		...
		{Schema of the magnetosphere with emphasis on open (blue lines) and closed (red lines) magnetic flux during times of southern IMF. The magnetopause is indicated by the dashed line. The opening flux rate at the dayside is denoted with $\Phi_\text{D}$ and closing at the nightside with $\Phi_\text{N}$. The inset shows day- and nightside of the Earth and the relation between auroral ovals (yellow areas) and the open flux at the polar caps ($F_\text{PC}$). The ring current (green arrow) intensity $H_\text{SYM}$. Credit: {\citet[Fig.~1]{Milan2009}}, \textcopyright~American Geophysical Union, reproduced with permission.}
...
\end{align}
where v is the velocity of the solar wind; |B| is the magnetic field magnitude; and the interplanetary magnetic field (IMF) clock angle is defined by $\theta_c = \tan^{-1}(By/Bz)$. The correlation coefficient of $d\phi/dt$ with the \Kp~index is $r = 0.76$ [see Newell et al., 2007, Table 3]. The predicted magnetic field time series in GSM coordinate system is used to calculate a theoretical magnetic flux rate. A Kp prediction is then generated by the empirical correlation.\\
\begin{align}
...
Savani2017:\\
Extended periods of southward magnetic field, often found within and surrounding Earth-directed CMEs, result in periods of enhanced reconnection on Earth’s dayside and energy input into the magnetosphere. In contrast, northward directed fields inhibit reconnection, resulting in a weaker magnetospheric response \citep{Dungey1961}. As CMEs usually generate the most geoeffective events [e.g., Tsurutani and Gonzalez, 1997; Zhang et al., 2007; Zhang and Moldwin, 2014], inferring the magnetic field direction along the trajectory of Earth through the CME would be a major advance in geomagnetic activity prediction.\\
...
constant-alpha force-free magnetic clouds \citep{Burlaga1988}:\\
\citep{Burlaga1988}: ``The aim of this paper is simply to determine whether or not Lundquist's solution for a cylindrically symmetric force-free magnetic field with constant alpha can describe the types of signatures observed in the solar wind at 1 AU when magnetic clouds move past a spacecraft.''\\
\citep{Burlaga1988}: ``By minimizing the magnetic energy subject to the constraint of constant magnetic helicity, Woltjer [1958] has shown that a force-free field with constant alpha represents the state of lowest magnetic energy which a closed ideal magnetohydrodynamic (MHD) system may attain.''\\
