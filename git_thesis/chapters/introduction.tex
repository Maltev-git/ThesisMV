...
\begin{itemize*}
	...
	\item $\vect{E}$, the solar wind electrical field which is with the help of some assumptions often reduced to $E_\text{y}$. It is one of the most prominent coupling functions and represents the rate at which southward magnetic flux is transported to the magnetosphere \citep[p.~103]{Russell2007}:
	\begin{align}
	...
	\item $\text{d}\Phi_\text{MP} / \text{d}t$, a relation derived by \citet{Newell2007} that represents the rate magnetic flux is opened at the magnetopause. It shows the highest correlation with 9 out of 10 geomagnetic indices out of various tested coupling functions, aiming to be an universal solar wind-magnetosphere coupling function.
	\begin{align}
		\frac{\text{d}\Phi_\text{MP}}{\text{d}t} &= v^{4/3} |\vect{B}|^{2/3} \sin^{8/3}\left(\frac{\theta_c}{2}\right)	\label{eq:newell_merging_function}
	\end{align}
	The value of the exponent, $n = 8/3$, is determined empirically \citep{Newell2007}.
	
	\item $B_\text{s} v^2$, the power density in the solar wind, containing the kinetic energy of the bulk flow \citep{Lockwood2013}.
