...
\begin{figure}[htb]
	...
	\fcapside[\FBwidth]{
		...
		\caption{Total solar eclipse image of the inner corona up to 5~solar radii. The picture was taken in Mongolia, 1~August~2008 and is processed from multiple images. Visible are the for a quiet Sun in cycle minimum typical magnetic field's dipole structure and the equatorial streamer belt. Credit: \href{http://www.zam.fme.vutbr.cz/~druck/Eclipse/}{Miloslav Druckmüller, Peter Aniol, Jan Sládeček}, 2008, reproduced with permission.}
...
\begin{figure}[htb]
	...
	\fcapside[\FBwidth]{
		...
		\caption{Schema of the heliosphere and its surrounding flow structure. The heliosphere is formed by the interaction of the solar wind with the local interstellar medium at the heliopause. \citep[Credit:][Fig.~9]{Owens2013}}
...
Helioseismic measurements reveal that the large-scale convective flow is agglomerated into large convection cells with slow meridional flows of a few \si{\m\per\s}, see \autoref{fig:Zhao2013_meridional_flow}. A poleward subsurface flow and equatorward backflow beneath with a further poleward flow below are detected within each hemisphere, comprising a stacked double-cell profile. This meridional circulation has an average period of about 22~years. The plasma carries the magnetic flux, which emerges at the surface with the same periodicity.\\
...
In the transition phase toroidal magnetic flux emerges in belts above and below the solar equator, manifesting as bipolar active regions, resulting in a toroidal/multipolar structured magnetic field.\\


%%% butterfly pattern
Observations of the surface radial magnetic field show the appearance of bipolar magnetic flux at belts of about \SI{+-20}{\degree} latitude at the beginning of a cycle and a shift towards lower latitudes at the end of a cycle. Thus the plot of surface magnetic field over latitude and time reveals a butterfly pattern, see \autoref{fig:Hathaway_magbfly}. This butterfly pattern appears in sunspot observations as well \citep{Maunder1904}.\\
...
The emerging flux (whose leading polarity alternates with each cycle) is carried by the slow meridional surface flow poleward, canceling the current dominating polar field polarity and eventually resulting in the polar field switch.\\
\begin{figure}[htb]
	\centering
...
The SSN observations show large variations in cycle length (9--14~years) and amplitude (peak SSNs in the range 0--350).
...
``the variations in the meridional circulation are the source of variations in sunspot cycle amplitudes - neither the differential rotation (the omega effect) nor the convective motions (one source of the alpha effect and the source of diffusion) vary as much.'' Hathaway website\\
``the meridional flow is faster at solar cycle minimum and slower at maximum`` Hathaway website\\

''Other factors (many of which are stochastic in nature), the rate of Active Region emergence, the tilt of the active regions, the details of the random interactions between the convective motions and magnetic field have an impact on this period.`` Upton forum\\
...
near-Sun field configuration changes\\
out to heliopause.\\
sec:solar wind\\
...
\begin{figure}[htb]
	...
	\caption{Solar wind velocity and magnetic field polarity over latitude during low and high solar activity for the three orbits of the Ulysses spacecraft. The SSN and CS tilt angle are plotted as well. \citep[Credit:][Fig.~1]{McComas200809}. ask for high res. image and permission...}
...
Photosphere: magnetic bright points (MBP) \SIrange{1}{2}{\kilo G}\\ %Cranmer2005
...
\begin{figure}[htb]
	\begin{floatrow}
		\ffigbox{
			...
			\caption{Solar magnetic field geometry from the DQCS model with field lines (solid) and constant field strength surfaces (dashed). The quadrupole part allows equatorial outflows along the current sheet. \citep[Credit:][Fig.~3, reproduced with permission \textcopyright~ESO.]{Banaszkiewicz1998}}
