...
\begin{itemize*}
	\item \citet{Newell2008} developed a relation comprising the previously mentioned set of merging and viscous coupling functions:
	\begin{align}
		\Kp{} = 0.05 + \num{2.244e-4} \frac{\text{d}\Phi}{\text{d}t} + \num{2.844e-6} \sqrt{n} \cdot v^2	\,.
	\end{align}
	This equation for the least variance linear prediction of \Kp{} results in a correlation coefficient as high as $r = 0.866$??.
	
	\item Savani2017:\\
	``The correlation coefficient of $\text{d}\Phi / \text{d}t$ with the \Kp~index is $r = 0.76$?? [see Newell et al., 2007, Table 3]. A Kp prediction is then generated by the empirical correlation:''
	\begin{align}
		\Kp = 9.5 - \exp\left(2.18 - \num{5.20e5} \left(\frac{\text{d}\Phi}{\text{d}t}\right)\right)
	\end{align}
	``with the velocity and magnetic field measured in \si{\km\per\s} and \si{\nano\tesla}, respectively [Emmons et al., 2013; Mays et al., 2015a].''\\

	make figure that compares both Newell implementations...\\

	\item Kp(v)\\
	
	\item Kp(v)\\
	implement both into chapter2...\\
	
\end{itemize*}


from a fit using an appropriate type of a fit function.\\
This thesis adds more empirical relations to the \Kp~index -- for the cases when only the solar wind velocity $v$ or its electric field \vBz{} are known.\\
In this work I settle and work with the electric field \vBz{} as the coupling function, because...\\
